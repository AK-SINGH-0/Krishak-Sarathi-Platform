# Krishak Sarathi

Krishak Sarathi is a React-based smart agriculture platform designed to assist farmers with AI-powered voice interactions. The application provides crop recommendations, weather forecasts, mandi (market) prices, government scheme information, and multilingual support through a simple and intuitive user interface.

Krishak Sarathi
AI-Powered Voice Crop Advisor for Farmers

Copyright (c) 2026 Sushil Kumar and Team.
All Rights Reserved.

This software, including its source code, design, user interface, documentation,
graphics, AI workflows, database structure, and all related materials, is the
exclusive intellectual property of Sushil Kumar and Team.

NO PERMISSION IS GRANTED TO:

• Copy or reproduce this project or any part of it.
• Modify, reverse engineer, or create derivative works.
• Publish, distribute, sell, sublicense, or share this project.
• Use this project for commercial, educational, research, or personal purposes
without prior written permission from the copyright owner.
• Upload this project or its source code to any public or private repository,
website, cloud storage, or other platform without authorization.

Any unauthorized copying, distribution, modification, or use of this project
may constitute copyright infringement and may result in legal action under
applicable copyright and intellectual property laws.

This project is provided only to authorized collaborators for development and
review purposes. Access to the source code does not grant ownership or any
license to copy, reuse, or redistribute the project.

Project Owner:
Sushil Kumar

Development Team:
• Sushil Kumar – Frontend Development & Project Lead
• Ankush Kumar – Presentation, Documentation & Content
• Bijanshu Yadav – AI/ML & Backend Development

For permissions or licensing inquiries, contact the Project Owner directly.

ALL RIGHTS RESERVED.

Team Name:
Ankush Kumar
Sushil Kumar
Bijanshu Yadav
 //====================================================================================\\
||  KRISHAK SARATHI TEAM IS ALWAYS READY TO ASSIST WITH ANY FARMING RELATED ISSUES. 🌱  ||
 \\====================================================================================//
